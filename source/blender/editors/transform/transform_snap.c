...
static bool snapDerivedMesh(short snap_mode, ARegion *ar, Object *ob, DerivedMesh *dm, BMEditMesh *em, float obmat[4][4],
                            ...
                            const float mval[2], float r_loc[3], float r_no[3], float *r_dist_px, float *r_depth, bool setmanip)
{
	bool retval = false;
                       ...
                       const float mval[2], float r_loc[3], float r_no[3], float *r_dist_px, float *r_depth, bool setmanip)
{
	...
	if (ob->type == OB_MESH) {
		...
		retval = snapDerivedMesh(snap_mode, ar, ob, dm, em, obmat, ray_start, ray_normal, ray_origin, mval, r_loc, r_no, r_dist_px, r_depth, setmanip);
                           ...
                           const float mval[2], float *r_dist_px, float r_loc[3], float r_no[3], SnapMode mode)
{
	...
	if (mode == SNAP_ALL && obedit) {
		...
		retval |= snapObject(scene, snap_mode, ar, ob, ob->obmat, true,
		                     r_ob, r_obmat,
		                     ray_start, ray_normal, ray_origin, mval, r_loc, r_no, r_dist_px, r_ray_dist, setmanip);
	}

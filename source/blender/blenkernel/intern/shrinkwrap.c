...
static void shrinkwrap_calc_normal_projection(ShrinkwrapCalcData *calc, bool forRender)
{
	...
	if (bvhtree_from_mesh_faces(&treeData, calc->target, 0.0, 4, 6, true) &&
	    (auxMesh == NULL || bvhtree_from_mesh_faces(&auxData, auxMesh, 0.0, 4, 6, true)))
	{
...
static void shrinkwrap_calc_nearest_surface_point(ShrinkwrapCalcData *calc)
{
	...
	bvhtree_from_mesh_faces(&treeData, calc->target, 0.0, 2, 6, true);
	if (treeData.tree == NULL) {
		OUT_OF_MEMORY();
		return;
	}

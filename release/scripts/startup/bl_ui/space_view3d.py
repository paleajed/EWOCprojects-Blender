...
class VIEW3D_PT_view3d_meshdisplay(Panel):
    ...
    def draw(self, context):
        ...
        if toolsettings.use_presel:
        	row.prop(context.scene.tool_settings, "show_presel_normals", text="", icon='PRESEL')

        sub = row.row(align=True)
        sub.active = mesh.show_normal_vertex or mesh.show_normal_face or mesh.show_normal_loop or (toolsettings.use_presel and context.scene.tool_settings.show_presel_normals)
